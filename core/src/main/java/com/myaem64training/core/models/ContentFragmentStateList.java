package com.myaem64training.core.models;
...
public class ContentFragmentStateList {
	...
	/*
	 * public List<ContentFragmentMetroList> getMetroList() { return Arrays
	 * .asList((String[]) contentFragment.map(cf ->
	 * cf.getElement("metroList")).map(ContentElement::getValue)
	 * .map(FragmentData::getValue).orElse(new String[0]))
	 * .stream().map(metroListPath ->
	 * resource.getResourceResolver().resolve(metroListPath))
	 * .filter(Objects::nonNull) .map(metroListResource ->
	 * metroListResource.adaptTo(ContentFragmentMetroList.class))
